...
class ChatModel
{
     ...
     */
    public function verificarAcessoMidia($usuario_id, $caminho_relativo)
    {
        try {
            $sql = "SELECT c.* FROM conversas c 
                    INNER JOIN mensagens_chat m ON c.id = m.conversa_id 
                    WHERE c.usuario_id = :usuario_id 
                    AND (m.midia_url LIKE :caminho OR m.conteudo LIKE :caminho)
                    LIMIT 1";

            $this->db->query($sql);
            $this->db->bind(':usuario_id', $usuario_id);
            $this->db->bind(':caminho', '%' . $caminho_relativo . '%');
    ...
    /**
     * Lista mídias de uma conversa
     */
    public function listarMidiasConversa($conversa_id)
    {
        try {
            $sql = "SELECT m.*, 
                    CASE 
                        WHEN m.midia_url IS NOT NULL THEN m.midia_url
                        WHEN m.conteudo LIKE 'uploads/%' THEN CONCAT('" . URL . "/public/', m.conteudo)
                        ELSE NULL
                    END as url_visualizacao,
                    CASE 
                        WHEN m.tipo IN ('image', 'audio', 'video', 'document') THEN m.tipo
                        ELSE NULL
                    END as tipo_midia
                    FROM mensagens_chat m
                    WHERE m.conversa_id = :conversa_id 
            ...
            error_log("Erro ao listar mídias da conversa: " . $e->getMessage());
            return [];
        }
    }

    /**
     * Busca mídias antigas para limpeza
     */
    public function buscarMidiasAntigas($diasParaManter = 90)
    {
        try {
            ...
            $sql = "SELECT m.*, 
                    ...
                    FROM mensagens_chat m
                    WHERE m.enviado_em < :data_limite 
                    AND m.tipo IN ('image', 'audio', 'video', 'document')
                    AND (m.midia_url IS NOT NULL OR m.conteudo LIKE 'uploads/%')";
            ...
            error_log("Erro ao buscar mídias antigas: " . $e->getMessage());
            return [];
        }
    }

    /**
     * Atualiza referência de arquivo removido
     */
    public function atualizarReferenciaArquivoRemovido($mensagem_id)
    {
        try {
            $sql = "UPDATE mensagens_chat SET 
                    conteudo = CONCAT('[ARQUIVO REMOVIDO] ', conteudo),
                    midia_url = NULL
                    WHERE id = :mensagem_id";

            $this->db->query($sql);
            $this->db->bind(':mensagem_id', $mensagem_id);
            return $this->db->executa();

        } catch (Exception $e) {
            error_log("Erro ao atualizar referência de arquivo removido: " . $e->getMessage());
            return false;
        }
    }

    /**
     * Conta total de mídias no sistema
     */
    public function contarTotalMidias()
    {
        try {
            $sql = "SELECT COUNT(*) as total 
                    FROM mensagens_chat 
                    WHERE tipo IN ('image', 'audio', 'video', 'document')
                    AND (midia_url IS NOT NULL OR conteudo LIKE 'uploads/%')";
            ...
            error_log("Erro ao contar total de mídias: " . $e->getMessage());
            return 0;
        }
    }

    /**
     * Busca estatísticas de mídias por tipo
     */
    public function buscarEstatisticasMidiasPorTipo()
    {
        try {
            $sql = "SELECT 
                        tipo,
                        COUNT(*) as quantidade,
                        COUNT(CASE WHEN midia_url IS NOT NULL OR conteudo LIKE 'uploads/%' THEN 1 END) as com_arquivo
                    FROM mensagens_chat 
                    WHERE tipo IN ('image', 'audio', 'video', 'document')
                    GROUP BY tipo
            ...
            error_log("Erro ao buscar estatísticas por tipo: " . $e->getMessage());
            return [];
        }
    }

    /**
     * Busca estatísticas de mídias por mês
     */
    public function buscarEstatisticasMidiasPorMes($limite = 12)
    {
        try {
            $sql = "SELECT 
                        DATE_FORMAT(enviado_em, '%Y-%m') as ano_mes,
                        COUNT(*) as quantidade,
                        COUNT(CASE WHEN tipo = 'image' THEN 1 END) as imagens,
                        COUNT(CASE WHEN tipo = 'audio' THEN 1 END) as audios,
                        COUNT(CASE WHEN tipo = 'video' THEN 1 END) as videos,
                        COUNT(CASE WHEN tipo = 'document' THEN 1 END) as documentos
                    FROM mensagens_chat 
                    WHERE tipo IN ('image', 'audio', 'video', 'document')
                    AND enviado_em >= DATE_SUB(NOW(), INTERVAL :limite MONTH)
                    GROUP BY DATE_FORMAT(enviado_em, '%Y-%m')
            ...
            error_log("Erro ao buscar estatísticas por mês: " . $e->getMessage());
            return [];
        }
    }
