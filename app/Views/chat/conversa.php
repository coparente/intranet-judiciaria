                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <?php if ($dados['conversa']->status_atendimento !== 'fechado'): ?>
                                                    ...
                                                    <a class="dropdown-item text-danger" href="#" data-toggle="modal" data-target="#modalEncerrarTicket">
                                                        <i class="fas fa-times-circle me-2"></i> Encerrar Ticket
                                                    </a>
                                                <?php else: ?>
                                                    <a class="dropdown-item text-success" href="#" data-toggle="modal" data-target="#modalReabrirTicket">
...
window.alterarStatusTicket = function(novoStatus) {
    console.log('Função alterarStatusTicket chamada com status:', novoStatus);
...
<script>
// Garantir que as funções de ticket funcionem
$(document).ready(function() {
    // Teste: verificar se as funções estão disponíveis
    console.log('Verificando funções de ticket...');
    console.log('alterarStatusTicket disponível:', typeof window.alterarStatusTicket);
    console.log('encerrarTicket disponível:', typeof window.encerrarTicket);
    console.log('reabrirTicket disponível:', typeof window.reabrirTicket);
    
    // Adicionar eventos de clique para os botões de status
    $(document).on('click', '[onclick*="alterarStatusTicket"]', function(e) {
        e.preventDefault();
        console.log('Clique detectado em botão de alterar status');
        const onclick = this.getAttribute('onclick');
        const match = onclick.match(/alterarStatusTicket\('([^']+)'\)/);
        if (match) {
            const status = match[1];
            console.log('Alterando status do ticket para:', status);
            alterarStatusTicket(status);
        }
    });
    
    // Adicionar evento de clique no botão de encerrar ticket
    $(document).on('click', '[data-target="#modalEncerrarTicket"]', function(e) {
        e.preventDefault();
        console.log('Clique no botão de encerrar ticket detectado');
        $('#modalEncerrarTicket').modal('show');
    });
    
    // Adicionar evento de clique no botão de reabrir ticket
    $(document).on('click', '[data-target="#modalReabrirTicket"]', function(e) {
        e.preventDefault();
        console.log('Clique no botão de reabrir ticket detectado');
        $('#modalReabrirTicket').modal('show');
    });
});
</script>
